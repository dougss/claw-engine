  ...
  const fetchTasks = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const fetchedTasks: Task[] = data.tasks || data.items || data;
        ...
        const sortedTasks = [...fetchedTasks].sort((a, b) => {
          ...
          if (aRunning) {
            ...
            const bTime = b.durationMs ? new Date(b.createdAt).getTime() + b.durationMs : new Date(b.createdAt).getTime();
            const aTime = a.durationMs ? new Date(a.createdAt).getTime() + a.durationMs : new Date(a.createdAt).getTime();
            return bTime - aTime;
          }
        });
        ...
        if (!selectedId) {
          const runningTask = sortedTasks.find(task => task.status === 'running');
