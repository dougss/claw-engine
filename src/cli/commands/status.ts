...
import { getWorkItemById, listWorkItems } from "../../storage/repositories/work-items-repo.js";
    ...
    .action(async (workItemId?: string) => {
      ...
      try {
        ...
        if (workItemId) {
            ...
            for (const task of tasks) {
              ...
              if (telemetry.length > 0) {
                ...
                for (const event of latestEvents) {
                  console.log(`      - ${event.eventType} (${new Date(event.createdAt).toLocaleString()})`);
                }
              }
              console.log('');
            }
          ...
          // Get all work items regardless of status
          const workItems = await listWorkItems(db);
        ...
        console.error('Error fetching status:', error instanceof Error ? error.message : String(error));
        process.exit(1);
      }
    });
}
