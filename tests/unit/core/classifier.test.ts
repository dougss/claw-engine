...
describe("classifyTask", () => {
  ...
  it('returns "medium" complexity when model responds with "medium"', async () => {
    vi.stubGlobal("fetch", mockFetch('{"complexity": "medium", "title": "add a new endpoint"}'));
    const result = await classifyTask("add a new endpoint", opts);
    expect(result.complexity).toBe("medium");
    expect(result.title).toBe("add a new endpoint");
  });

  it('returns "medium" as fallback on non-OK response', async () => {
    vi.stubGlobal("fetch", mockFetch("simple", false));
    const result = await classifyTask("any task", opts);
    expect(result.complexity).toBe("medium");
    expect(result.title).toBe("any task");
  });
  ...
  it("handles leading whitespace and uppercase in response", async () => {
    ...
    const result = await classifyTask("rename a constant", opts);
    expect(result.complexity).toBe("simple");
    expect(result.title).toBe("rename a constant");
  });
